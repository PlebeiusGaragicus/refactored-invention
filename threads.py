...
class Post:
    def __init__(self, body: str, user=None):
        self.body = body
        self.user = user
        self.children = []
        ...
        with st.popover(reply_text, use_container_width=True):
            with st.form(f"reply_form_{id(post)}", clear_on_submit=True):  # Unique form ID for each post
                ...
                if st.form_submit_button("Submit"):
                    ...
                    process_post(post, body=reply_text, user="satoshi")
                    st.rerun()
        ...
        if post.show_children:
            for child in post.children:
                with st.container(border=True):
                    ...
                    with cols[1]:
                        render_reply_form(child, indent)  # Render reply form for each post
                        # Recursively display replies, increasing indent

                    cmp_posts(child, indent + 4)
